...
@app.route('/register', methods=['GET', 'POST']) #/register is the name of the function that needs to be created, method you GET to retrieve user info and Post to send it to postman
def register():
...
def CreditCard():
    CC_num = request.form['Credit Card Number']
    test = User.query.filter_by(CC_num=CC_num).first()
    if test:
        return jsonify(message='That credit card already exists.'), 409

    else:
        creditCardNumber = request.form['Credit Card Number']
        CVV = request.form['CVV code']
        expirationDate = request.form['Expiration Date']
        CreditCard = User(creditCardNumber=creditCardNumber, CVV=CVV, expirationDate=expirationDate)
